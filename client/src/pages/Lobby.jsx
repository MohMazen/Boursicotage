...
export default function Lobby() {
    ...
    return (
        <div className="lobby-page">
            <AnimatedBackground />
            <h1 className="lobby-title">📈 Boursicotage</h1>
            <p className="lobby-pseudo">Connecté en tant que : <span>{pseudo}</span></p>

            <div className="lobby-card">
                <p className="lobby-subtitle">Joueurs connectés ({joueurs.length})</p>

                <ul className="lobby-liste">
                    {joueurs.map((joueur) => (
                        <li key={joueur.id} className="lobby-joueur">
                            <div className="lobby-joueur-dot"></div>
                            {joueur.name}
                        </li>
                    ))}
                </ul>

                <button className="lobby-button" onClick={() => navigate(`/game?pseudo=${pseudo}`)}>
                    Démarrer la partie
                </button>
            </div>
        </div>
